...
export default function Rooms() {
  ...
  const columns: Column<Room>[] = [
    ...
    },
    {
      ...
      render: (r) => (
        <StatusBadge
          status={r.privacy_mode === "private" ? "banned" : "active"}
          label={r.privacy_mode === "private" ? "Riêng tư" : "Công khai"}
        />
      ),
    },
    {
    ...
    <div className="space-y-8">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        <PageHeader
          ...
          description="Giám sát danh sách các phòng học nhóm trực tuyến của học viên"
        />
        ...
        message={
          <div className="space-y-4 text-left">
            <p className="text-slate-600">
              Bạn có chắc chắn muốn GIẢI TÁN phòng học "{selectedRoom?.name}"?
              Mọi tin nhắn nhóm và danh sách thành viên sẽ bị xóa vĩnh viễn khỏi
              database.
            </p>
            <div className="bg-red-50/50 p-4 rounded-xl border border-red-100">
              <label className="block text-xs font-bold uppercase tracking-wider text-red-800 mb-2">
                Lý do giải tán (Tùy chọn)
              </label>
              <textarea
                rows={2}
                value={reason}
              ...
              />
              <p className="text-[11px] text-red-600/80 mt-1.5 font-medium">
                * Trưởng phòng và các thành viên đang tham gia sẽ nhận được
                thông báo này ngay lập tức
              </p>
            </div>
          </div>
        }
