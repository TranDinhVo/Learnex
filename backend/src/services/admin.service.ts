...
export const adminService = {
  ...
  async banUser(id: string) {
    ...
    if (user.role === "admin")
      throw new AppError("Cannot ban an administrator.", 400);
  ...
  async deleteUser(id: string) {
    ...
    if (!user) throw new AppError("User not found.", 404);
    if (user.role === "admin")
      throw new AppError("Cannot delete an administrator.", 400);
  ...
  async hidePost(id: string, reason?: string) {
    ...
    const finalReason =
      reason && reason.trim() ? reason.trim() : "Vi phạm Tiêu chuẩn cộng đồng";
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          [user.fcm_token],
          "Bài viết bị ẩn",
          `Bài viết của bạn đã bị ẩn khỏi cộng đồng. Lý do: ${finalReason}`,
          { type: "system", ref_type: "post", ref_id: id },
        )
        .catch(console.error);
    }
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          [user.fcm_token],
          "Bài viết đã bị xóa",
          `Bài đăng của bạn đã bị xóa vĩnh viễn khỏi hệ thống. Lý do: ${finalReason}`,
          { type: "system" },
        )
        .catch(console.error);
    }
  ...
  async getAllDocuments(pagination: PaginationParams) {
    ...
    if (pagination.search) {
      const searchPattern = `%${pagination.search}%`;
      query = query
        .where("d.title", "ILIKE", searchPattern)
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          ...
          `Tài liệu "${doc.title}" của bạn đã được phê duyệt và hiển thị công khai.`,
          { type: "system", ref_type: "document", ref_id: id },
        )
        .catch(console.error);
    }
  ...
  async rejectDocument(id: string, reason?: string) {
    ...
    const finalReason =
      reason && reason.trim()
        ? reason.trim()
        : "Không đạt chất lượng chuyên môn";
    await notificationService.create({
      user_id: doc.user_id,
      type: "system",
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          [user.fcm_token],
          "Tài liệu bị từ chối",
          `Tài liệu "${doc.title}" của bạn đã bị từ chối. Lý do: ${finalReason}`,
          { type: "system", ref_type: "document", ref_id: id },
        )
        .catch(console.error);
    }
  ...
  async deleteDocument(id: string, reason?: string) {
    ...
    const finalReason =
      reason && reason.trim() ? reason.trim() : "Vi phạm quy định nền tảng";
    await notificationService.create({
      user_id: doc.user_id,
      type: "system",
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          [user.fcm_token],
          "Tài liệu đã bị xóa",
          `Tài liệu "${doc.title}" của bạn đã bị xóa vĩnh viễn khỏi hệ thống. Lý do: ${finalReason}`,
          { type: "system" },
        )
        .catch(console.error);
    }
    ...
    const data = await Promise.all(
      rawData.map(async (r) => {
        ...
        return {
          ...
          isActive: true,
          isPrivate: r.privacy_mode === "private",
          maxMembers: 50,
  ...
  async deleteRoom(id: string, reason?: string) {
    ...
    if (user && user.fcm_token) {
      fcmService
        .sendMultiplePush(
          [user.fcm_token],
          "Phòng học đã bị giải tán",
          `Phòng học "${room.name}" của bạn đã bị Admin giải tán. Lý do: ${finalReason}`,
          { type: "system" },
        )
        .catch(console.error);
    }
  ...
  async sendNotification(
    ...
    if (tokens.length > 0) {
      fcmService
        .sendMultiplePush(tokens, data.title, data.message, { type: "system" })
        .catch((err) => {
          console.error(
            "Lỗi khi gửi Push Notification trong Admin Service:",
            err,
          );
        });
    }
